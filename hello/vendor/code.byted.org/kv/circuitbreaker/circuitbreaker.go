...
type Breaker struct {
	...
	// halfopen state success times needed
	NeededSuc int
...
func NewBreakerWithOptions(options *Options) *Breaker {
	...
	if options.NeededSuc == 0 {
		options.NeededSuc = defaultNeededSuccesses
	}

	return &Breaker{
		BackOff:     options.BackOff,
		NeededSuc:   options.NeededSuc,
		Clock:       options.Clock,
...
func (cb *Breaker) Success() {
	...
	if state == halfopen {
		...
		if atomic.LoadInt64(&cb.halfOpens) >= int64(cb.NeededSuc) {
			cb.Reset()
		}
	}
	atomic.StoreInt64(&cb.consecFailures, 0)
