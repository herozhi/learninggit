...
func NewIScanCmd(process func(cmd Cmder) error, args ...interface{}) *IScanCmd {
	return &IScanCmd{
		baseCmd: baseCmd{_args: args},
		// process: process,
	}
...
func (cmd *IScanCmd) Val() (keys []string, cursor string) {
	return cmd.page, cmd.cursor
}

func (cmd *IScanCmd) Result() (keys []string, cursor string, err error) {
	return cmd.page, cmd.cursor, cmd.err
}

func (cmd *IScanCmd) String() string {
	return cmdString(cmd, cmd.page)
}

func (cmd *IScanCmd) readReply(cn *pool.Conn) error {
	p := cn.Rd
	n, err := p.ReadArrayLen()
	if err != nil {
		cmd.page, cmd.cursor, cmd.err = nil, "", err
		return err
	}
	if n != 2 {
		cmd.page, cmd.cursor, cmd.err = nil, "", fmt.Errorf("redis: got %d elements in iscan reply, expected 2", n)
		return err
	}

	cursor, err := p.ReadStringReply()
	if err != nil {
		cmd.page, cmd.cursor, cmd.err = nil, "", err
		return err
	}
	...
	cmd.page, cmd.cursor, cmd.err = keys, cursor, err
	return cmd.err
}
...
func (cmd *XGetCmd) SetVal(val interface{}, generation int64) {
	cmd.val = val
...
func (cmd *XSetCmd) SetVal(numOps int64, val interface{}, generation int64) {
	cmd.numOps = numOps
	cmd.val = val
	cmd.generation = generation
}
...
func (cmd *ScanRowCmd) SetVal(val interface{}) {
	cmd.val = val
}
...
func (cmd *TtlQScanCmd) SetVal(val interface{}) {
	cmd.val = val
}
