...
type Logger struct {
	...
	isRunning        int32
	level            int32
	syncProcessLevel int32
	buf              chan *LogMsg
	processBuf       chan *LogMsg
...
func NewLogger(bufLen int) *Logger {
	...
	l := &Logger{
		level:            LevelDebug,
		syncProcessLevel: LevelWarn,
		buf:              make(chan *LogMsg, bufLen),
		processBuf:       make(chan *LogMsg, bufLen),
...
// SetSyncProcessLevel .
func (l *Logger) SetSyncProcessLevel(level int) {
	atomic.StoreInt32(&l.syncProcessLevel, int32(level))
}

// GetSyncProcessLevel .
func (l *Logger) GetSyncProcessLevel() int {
	return int(atomic.LoadInt32(&l.syncProcessLevel))
}
...
func (l *Logger) StartLogger() {
	...
	LOG_PROCESS:
		for {
			select {
			case log, ok := <-l.processBuf:
				if !ok {
					fmt.Fprintln(os.Stderr, "processBuf channel has been closed.")
					return
				}

				for _, processor := range l.processors {
					tmpLog, tmpKvs, validated := processor(log.Msg, log.Kvs...)
					if !validated {
						continue LOG_PROCESS
					}
					log.Msg = tmpLog
					log.Kvs = tmpKvs
				}
				if len(l.providers) > 0 || len(l.providerPluses) > 0 {
					l.fmtForProvider(log)
				}
				if len(l.kvproviders) > 0 {
					l.fmtForKVProvider(log)
				}
			case <-l.stop:
				return
			}
...
func (l *Logger) fmtLog(ctx context.Context, level int, fileLocation string, rawLog string, kvs ...interface{}) {
	...
	if len(fileLocation) == 0 && l.callDepth > 0 {
		fileLocation = location(l.callDepth+1, l.useFullPath)
	}
	...
	if level >= int(atomic.LoadInt32(&l.syncProcessLevel)) {
		l.processBuf <- msg
	} else {
		select {
		case l.processBuf <- msg:
		default:
		}
	}
}

func (l *Logger) fmtForProvider(log *LogMsg) {
	select {
	case l.buf <- log:
	default:
	}
...
func (l *Logger) fmtForKVProvider(log *LogMsg) {
	headers := make(map[string]string, 9)
	headers["level"] = string(levelBytes[log.Level])
	headers["timestamp"] = strconv.Itoa(int(log.Time.UnixNano() / int64(time.Millisecond)))
	headers["location"] = log.Location
	headers["host"] = env.HostIP()
	headers["psm"] = l.PSM
	headers["cluster"] = env.Cluster()
	headers["logid"] = string(utils.LogIDFromContext(log.Ctx))
	headers["stage"] = env.Stage()
	headers["pod_name"] = env.PodName()
	headers["span_id"] = strconv.FormatUint(utils.SpanIDFromContext(log.Ctx), 16)

	kvMap := make(map[string]string, len(log.Kvs))
	for i := 0; i+1 < len(log.Kvs); i += 2 {
		k := log.Kvs[i]
		v := log.Kvs[i+1]
