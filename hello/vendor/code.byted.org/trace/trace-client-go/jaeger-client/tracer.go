...
const (
	// ParentOfRef refers to a child Span that caused *and* somehow depends
	// upon the new parent Span.
...
type Tracer struct {
	...
	options struct {
		...
		postTraceIDGenerator func(options opentracing.StartSpanOptions) TraceID // custom trace ID generator for tail-based trace
...
func (t *Tracer) startSpanWithOptions(
	...
	newTrace := false
	if !hasParent || !parent.IsValid() {
		isPostTrace := options.Tags[PostTraceTag.Key] == PostTraceTag.Value
		if isPostTrace {
			ctx.traceID = t.options.postTraceIDGenerator(options)
		...
		if options.Tags[PostTraceRootSpanTag.Key] != PostTraceRootSpanTag.Value &&
			isPostTrace {
			ctx.parentID = SpanID(t.randomID())

			// mock parent context for post trace, Attention! parentRef.Context not complete！
			parentRef := new(Reference)
			parentRef.Type = opentracing.ChildOfRef
			parentRef.Context.traceID, parentRef.Context.spanID = ctx.traceID, ctx.parentID
			references = append(references, *parentRef)
		...
		if rpcServer && t.options.zipkinSharedRPCSpan {
			...
			if hasChild {
				samplerTags = PostTraceFromDownstremTag
			}

			if hasChild && firstChild.ParentID() != 0 {
				ctx.spanID = firstChild.ParentID()
			} else {
				ctx.spanID = SpanID(t.randomID())
			}
	...
	// 在此统一做 无效检查 和 span级限流
	if ctx.IsSampled() && (!ctx.IsValid() || !t.isDebugAllowed(operationName)) {
...
func (t *Tracer) startSpanInternal(
	...
	if tags[PostTraceTag.Key] == PostTraceTag.Value {
		t.metrics.PostSpansStarted.Inc(1)
		if newTrace {
			t.metrics.PostTracesStarted.Inc(1)
		}
	}
	return sp
}
...
func (t *Tracer) reportSpan(sp *Span) {
	...
	if sp.IsSampled() {
		...
		t.logger.Infof("span reporting: span-ctx: %v", sp.Context())
