...
func (c *cmdable) LDrop(key string) *IntCmd {
	keys := []string{key}
	return c.del("ldrop", keys...)
}

func (c *cmdable) SDrop(key string) *IntCmd {
	keys := []string{key}
	return c.del("sdrop", keys...)
}
...
func (c *cmdable) HSetExpire(key string, expiration time.Duration) *BoolCmd {
	cmd := NewBoolCmd("hsetexpires", key, formatSec(expiration))
	c.process(cmd)
	return cmd
}

func (c *cmdable) ZSetExpire(key string, expiration time.Duration) *BoolCmd {
	cmd := NewBoolCmd("zsetexpires", key, formatSec(expiration))
	c.process(cmd)
	return cmd
}

func (c *cmdable) LSetExpire(key string, expiration time.Duration) *BoolCmd {
	cmd := NewBoolCmd("lsetexpires", key, formatSec(expiration))
	c.process(cmd)
	return cmd
}

func (c *cmdable) SSetExpire(key string, expiration time.Duration) *BoolCmd {
	cmd := NewBoolCmd("ssetexpires", key, formatSec(expiration))
	c.process(cmd)
	return cmd
}
...
func (c *cmdable) HTTL(key string) *DurationCmd {
	cmd := NewDurationCmd(time.Second, "httl", key)
	c.process(cmd)
	return cmd
}

func (c *cmdable) ZTTL(key string) *DurationCmd {
	cmd := NewDurationCmd(time.Second, "zttl", key)
	c.process(cmd)
	return cmd
}

func (c *cmdable) STTL(key string) *DurationCmd {
	cmd := NewDurationCmd(time.Second, "sttl", key)
	c.process(cmd)
	return cmd
}

func (c *cmdable) LTTL(key string) *DurationCmd {
	cmd := NewDurationCmd(time.Second, "lttl", key)
	c.process(cmd)
	return cmd
}
...
func (c *cmdable) GetShardCount() *IntCmd {
	cmd := NewIntCmd("getshardcount")
	c.process(cmd)
	return cmd
}

func (c *cmdable) IScan(shard uint64, cursor string, match string, count int64) *IScanCmd {
	if cursor == "" {
		cursor = "0"
	}
	args := []interface{}{"iscan", shard, cursor}
	if match != "" {
	...
	cmd := NewIScanCmd(c.process, args...)
	c.process(cmd)
	return cmd
}
...
func (c *cmdable) XDecrBy(key string, decrement int64) *IntCmd {
	cmd := NewIntCmd("xdecrby", key, decrement)
	c.process(cmd)
	return cmd
}
...
// bytedance international counter service
...
func (c *cmdable) HXDecrBy(key, field string, decrement int64) *IntCmd {
	cmd := NewIntCmd("hxdecrby", key, field, decrement)
	c.process(cmd)
	return cmd
}
