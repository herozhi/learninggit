...
func (p *textMapPropagator) Extract(abstractCarrier interface{}) (SpanContext, error) {
	...
	// sacrifice not case sensitive and only baggage case of TextMapCarrier for performance
	textMapCarrier, ok := abstractCarrier.(opentracing.TextMapCarrier)
	if ok && textMapCarrier != nil && textMapCarrier[p.headerKeys.JaegerDebugHeader] == "" && textMapCarrier[p.headerKeys.TraceContextHeaderName] == "" {
