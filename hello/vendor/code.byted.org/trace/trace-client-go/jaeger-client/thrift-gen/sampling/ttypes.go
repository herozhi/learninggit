...
type PerOperationSamplingStrategies struct {
	...
	DefaultUpperBoundTracesPerSecond *float64                     `thrift:"defaultUpperBoundTracesPerSecond,4" json:"defaultUpperBoundTracesPerSecond"`
}
...
func (p *PerOperationSamplingStrategies) writeField4(oprot thrift.TProtocol) (err error) {
	if p.IsSetDefaultUpperBoundTracesPerSecond() {
		if err := oprot.WriteFieldBegin("defaultUpperBoundTracesPerSecond", thrift.DOUBLE, 4); err != nil {
			return fmt.Errorf("%T write field begin error 4:defaultUpperBoundTracesPerSecond: %s", p, err)
		}
		if err := oprot.WriteDouble(float64(*p.DefaultUpperBoundTracesPerSecond)); err != nil {
			return fmt.Errorf("%T.defaultUpperBoundTracesPerSecond (4) field write error: %s", p, err)
		}
		if err := oprot.WriteFieldEnd(); err != nil {
			return fmt.Errorf("%T write field end error 4:defaultUpperBoundTracesPerSecond: %s", p, err)
		}
	}
	return err
}
